...
/**
 * GET /api/cron/refresh-market-orderbooks
 * Prewarms market orderbook cache used by Sharp Projections.
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL("/api/market-orderbooks", req.nextUrl.origin)
    url.searchParams.set("sport", "all")
    url.searchParams.set("marketType", "all")
    url.searchParams.set("limit", "240")
    url.searchParams.set("depth", "8")
    url.searchParams.set("minSharpNotional", "2000")
      ...
      return NextResponse.json(
        {
          ok: false,
          refreshed: false,
          error: payload?.error || "Failed to refresh market orderbooks cache.",
        },
        { status: res.status }
      )
    }
    ...
    return NextResponse.json({
      ...
      marketType: payload?.marketType ?? "all",
      count: typeof payload?.count === "number" ? payload.count : null,
      updatedAt: payload?.updatedAt ?? null,
    })
  } catch (error: any) {
    console.error("[Cron: Market Orderbooks] Fatal error:", error)
