...
import { setSharpTradersCache } from '@/lib/services/polymarket-sharp-traders-cache'
...
export async function GET(req: NextRequest) {
  try {
    ...
    for (let page = 0; page < DEFAULT_TRADE_PAGES; page += 1) {
      const offset = page * DEFAULT_TRADE_LIMIT
      const trades = await fetchTradesPage(DEFAULT_TRADE_LIMIT, offset)
      for (const trade of trades) {
        if (trade.proxyWallet) walletSet.add(trade.proxyWallet.toLowerCase())
      }
      fetchedTrades += trades.length
      if (trades.length < DEFAULT_TRADE_LIMIT || fetchedTrades >= DEFAULT_MIN_TRADE_SAMPLES) break
    }

    const wallets = Array.from(walletSet)
    if (wallets.length === 0) {
      return NextResponse.json({ ok: false, error: 'No wallets found.' }, { status: 500 })
    }
    ...
    const cached = await setSharpTradersCache(FULL_CACHE_KEY, payload)
