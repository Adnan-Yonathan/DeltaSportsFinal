...
import { Menu, X, Activity, Radar, LineChart, ShieldCheck, Zap, Target, BarChart3 } from 'lucide-react'
...
const menuItems = [
  ...
  { name: 'Calculator', href: '#calculate-your-edge' },
  { name: 'Results', href: '#betproof-showcase' },
]
    ...
    <div className="relative overflow-hidden border-b border-white/10 bg-black text-white">
      <header>
        <nav
          ...
          className="group relative z-20 w-full border-b border-white/10 bg-black/80 backdrop-blur"
        >
          <div className="mx-auto max-w-6xl px-6">
            <div className="flex flex-wrap items-center justify-between gap-6 py-3 lg:gap-0 lg:py-4">
              ...
              <div className="bg-zinc-950 group-data-[state=active]:block mb-6 hidden w-full flex-wrap items-center justify-end space-y-8 rounded-3xl border border-white/10 p-6 shadow-2xl shadow-black/50 md:flex-nowrap lg:m-0 lg:flex lg:w-fit lg:gap-6 lg:space-y-0 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none">
                  ...
                  <ul className="space-y-6 text-base lg:flex lg:gap-8 lg:space-y-0 lg:text-sm">
                    {menuItems.map((item) => (
                      <li key={item.name}>
                        <Link
                          href={item.href}
                          className="block text-zinc-300 duration-150 hover:text-emerald-200"
                        >
                          <span>{item.name}</span>
                        </Link>
                      </li>
