...
5. **Caching & telemetry**
   - Cache per `(model_id, game context)` during the conversation; set `last_used_at` and emit PostHog events (`model_created`, `model_applied`).
...
- **Observability**: PostHog events and structured error logs for failed model applications.
