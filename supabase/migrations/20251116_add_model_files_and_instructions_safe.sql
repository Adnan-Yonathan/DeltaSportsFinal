...
-- Safe version that checks if objects exist before creating them

-- Add new columns to custom_models table (IF NOT EXISTS is safe)
ALTER TABLE public.custom_models
ADD COLUMN IF NOT EXISTS description TEXT,
ADD COLUMN IF NOT EXISTS instructions TEXT,
ADD COLUMN IF NOT EXISTS file_metadata JSONB DEFAULT '[]'::jsonb;

-- Create model_files table (IF NOT EXISTS is safe)
...
-- Indexes for model_files (IF NOT EXISTS is safe)
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND tablename = 'model_files'
    AND policyname = 'Users can view own model files'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
    AND tablename = 'model_files'
    AND policyname = 'Users can create own model files'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
    AND tablename = 'model_files'
    AND policyname = 'Users can update own model files'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
    AND tablename = 'model_files'
    AND policyname = 'Users can delete own model files'
  ) THEN
...
-- Comments for documentation (these are always safe to re-run)
